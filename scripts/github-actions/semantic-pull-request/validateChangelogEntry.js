...
// module.exports = async function validateChangelog({ github, restParameters, semanticResult, linkedIssues }) {
//   // // gh pr view https://github.com/emilyrohrbough/cypress/pull/4 --json files
//   // const { data } = await github.rest.pulls.listFiles(restParameters);
...
//   // gh pr view https://github.com/emilyrohrbough/cypress/pull/4 --json files
//   const { data } = await github.rest.commits.compare({
//     ...restParameters,
//     basehead: `${process.env.GITHUB_BASE_REF}...${process.env.GITHUB_HEAD_REF}`,
//   });

//   const hasChangeLogUpdate = data.files.find(fileDetails => fileDetails.filename === 'cli/CHANGELOG.md');
